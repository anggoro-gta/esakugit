                ...
                <tr>
                    <?php
                        if($jenis_anggaran=='per_perbup1'){
                            $jenis = 'PERUBAHAN PERATURAN BUPATI 1';
                        }
                        if($jenis_anggaran=='per_perbup2'){
                            $jenis = 'PERUBAHAN PERATURAN BUPATI 2';
                        }
                        if($jenis_anggaran=='per_perbup3'){
                            $jenis = 'PERUBAHAN PERATURAN BUPATI 3';
                        }
                        if($jenis_anggaran=='per_perbup4'){
                            $jenis = 'PERUBAHAN PERATURAN BUPATI 4';
                        }
                        if($jenis_anggaran=='pak'){
                            $jenis = 'PERUBAHAN ANGGARAN KEGIATAN (PAK)';
                        }
                    ?>
                    <th>LAPORAN REALISASI ANGGARAN <?=strtoupper($kategori)?> <?=$jenis?></th> 
                </tr>
                <?php if(isset($hslBid)){ ?>
                    <tr>
            ...
            <table border="1" cellspacing="0" width="100%" style="font-size:9px;line-height: 2">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" width="90px">JUMLAH PENGAJUAN<br>S/D BULAN LALU</th>
                        <th rowspan="2" width="90px">JUMLAH PENGAJUAN<br>BULAN INI</th>
                        <th colspan="2">JUMLAH REALISASI<br>S/D BULAN INI</th>
                ...
                <tbody>
                    <tr>
                        ...
                        <?php
                            $anggaran_all = $hslAwal->tot_anggaran_all;
                            if($jenis_anggaran=='per_perbup1'){
                                $anggaran_all = $hslAwal->tot_anggaran_perbup1;
                            }
                            if($jenis_anggaran=='per_perbup2'){
                                $anggaran_all = $hslAwal->tot_anggaran_perbup2;
                            }
                            if($jenis_anggaran=='per_perbup3'){
                                $anggaran_all = $hslAwal->tot_anggaran_perbup3;
                            }
                            if($jenis_anggaran=='per_perbup4'){
                                $anggaran_all = $hslAwal->tot_anggaran_perbup4;
                            }
                            if($jenis_anggaran=='pak'){
                                $anggaran_all = $hslAwal->tot_anggaran_pak_all;
                            }

                            // if($hslAwal->tot_anggaran_pak_all!=''){
                            //     $anggaran_all = $hslAwal->tot_anggaran_pak_all;
                            // }
                        ?>
                        <td style="text-align: right"><?=number_format($anggaran_all)?></td>
                        <?php $pngjLaluAwl=$hslLaluAwlPjd->pengajuan_sekarang_pd+$hslLaluAwlRkp->pengajuan_sekarang_rd;?>
                        <td style="text-align: right"><?=number_format($pngjLaluAwl)?></td>
                        <?php $pngjSkrgAwl=$hslSkrgAwlPjd->pengajuan_sekarang_pd+$hslSkrgAwlRkp->pengajuan_sekarang_rd;?>
                        <td style="text-align: right"><?=number_format($pngjSkrgAwl)?></td>
                        <td style="text-align: right;"><?=number_format($pngjLaluAwl+$pngjSkrgAwl)?></td>
                        <td style="text-align: center"><?=!empty($anggaran_all)?number_format((($pngjLaluAwl+$pngjSkrgAwl)/$anggaran_all)*100,2):'0'?> %</td>
                        <td style="text-align: right;"><?=number_format($anggaran_all-($pngjLaluAwl+$pngjSkrgAwl))?></td>
                    </tr>
                            ...
                            <?php foreach ((array)$arrKeg[$val->id_prog] as $valKeg) { ?>
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        // if($valKeg->tot_anggaran_pak_keg!=''){
                                        //     $anggaran_keg = $valKeg->tot_anggaran_pak_keg;
                                        // }
                                    ?>
                                    ...
                                    <?php foreach ((array)$arrSubKeg[$valKeg->id_keg] as $valSub) { ?>
                                        <tr>
                                            ...
                                            <?php
                                                ...
                                                // if($valSub->tot_anggaran_pak_sub_keg!=''){
                                                //     $anggaran_subKeg = $valSub->tot_anggaran_pak_sub_keg;
                                                // }
                                            ?>

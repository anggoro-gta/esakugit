                ...
                <div class="box-inner">
                    <div class="box-header well">
                        <div class="pull-right">
                            <a href="<?=base_url()?>Lembur/create" class="btn btn-sm btn-success" title="Buat Kwitansi"><i class="glyphicon glyphicon-plus"></i> Tambah</a>
                            <!-- <a class="btn btn-xs btn-warning" href="<?=base_url()?>Lembur/updateRekap" title="Buat Rekap"><i class="glyphicon glyphicon-share-alt"></i> Buat Rekap</a> -->
                            <?php //if($this->session->userdata("level")==1){ ?>
                                <!-- <a class="btn btn-xs btn-danger" href="<?=base_url()?>Lembur/deleteRekap" title="Hapus Rekap"><i class="glyphicon glyphicon-trash"></i> Hapus Rekap</a>                             -->
                            <?php //} ?>
                    ...
                    <div class="box-content">  
                        <div class="table-responsive">                     
                            <table class="table table-bordered table-striped" id="example2" style="font-size: 85%">
                                <thead>
                                    <tr>
                                        <th style="text-align: center;vertical-align: middle;" rowspan="2" width="3%">No</th>
                                        <th style="text-align: center;vertical-align: middle;" rowspan="2">Bl. SPJ</th>
                                        <!-- <th style="text-align: center;vertical-align: middle;" rowspan="2">No. TBP</th> -->
                                        ...
                                        <th style="text-align: center;vertical-align: middle;" rowspan="2">Sub Kegiatan</th>
                                        <th style="text-align: center" colspan="3">Tanggal</th>
                                        <th style="text-align: center;vertical-align: middle;" rowspan="2">Perihal</th>
                                        <th style="text-align: center;vertical-align: middle;" rowspan="2">Cetak</th>
...
<div class="modal fade slide-up disable-scroll" id="modal_rekap" role="dialog" aria-hidden="false">  
    <div class="modal-dialog" style="width: 35%;padding: 0px">
        <div class="modal-content">
            <form method="post" action="<?=base_url("Lembur/cetakRekap")?>" target="_blank" enctype="multipart/form-data" class="form-horizontal">
    ...
    <div class="modal-dialog" style="width: 35%;padding: 0px">
        <div class="modal-content">
            <form method="post" action="<?=base_url("KwitansiHR/updateBKU")?>" enctype="multipart/form-data" class="form-horizontal">
                ...
                <input type="hidden" name="formnya" value="Lembur">
                <div class="modal-footer">
                    <button class="btn btn-md btn-default pull-left inline" data-dismiss="modal">Batal</button>
...
$(document).ready(function() {
    ...
    var t = $("#example2").dataTable({
        ...
        processing: true,
        serverSide: true,
        ajax: {"url": "<?= base_url()?>Lembur/getDatatables", "type": "POST", "data": {
        ...
        columns: [
            ...
            {  "data": "tgl_kegiatan_dari",
               "orderable": false,
                "className" : "text-center"
            },
            {  "data": "tgl_kegiatan_sampai",
               "orderable": false,
                "className" : "text-center"
            },
            {  "data": "perihal",
            ...
            },
            {
                ...
                render : function (data, type, row) {
                    hsl = '';
                    hsl += '<a href="<?=base_url()?>Lembur/cetakST/'+data+'" target="_blank"  title="Surat Tugas">ST</a>';
                    hsl += ' | ';
                    hsl += '<a href="<?=base_url()?>Lembur/cetakDhPNS/'+data+'" target="_blank" title="Daftar Hadir">DH</a>';
                    hsl += ' | ';
                    hsl += '<a href="<?=base_url()?>Lembur/cetakDftrUpah/'+data+'" target="_blank" title="Daftar Pembayaran Upah">Upah</a>';
                    hsl += ' | ';
                    hsl += '<a href="<?=base_url()?>Lembur/cetakKwiMamin/'+data+'" target="_blank"  title="Cetak Kwitansi Mamin">Kwi Mamin</a>';
                    hsl += ' | ';
                    hsl += '<a href="<?=base_url()?>Lembur/cetakKwiUpah/'+data+'" target="_blank"  title="Cetak Kwitansi Upah">Kwi Upah</a>';
                    
                    hsl += ' | ';
                    hsl += '<a href="<?=base_url()?>Lembur/lapLembur/'+data+'" target="_blank"  title="Laporan Lembur">Lap</a>';
            ...
            {"data": "id",
                render : function ( data, type, row ){ 
                    ...
                    if(row.is_spj=='0'){
                        aksi += '<a class="btn btn-xs btn-danger" href="<?=base_url()?>Lembur/delete/'+data+'"><i class="glyphicon glyphicon-trash icon-white" title="Hapus" onclick="return confirmDelete()"></i></a>';
                        aksi += '<a class="btn btn-xs btn-success" href="<?=base_url()?>Lembur/update/'+data+'"><i class="glyphicon glyphicon-edit icon-white" title="Update"></i></a>';  
                    }
                    aksi += '<a class="btn btn-xs btn-warning" href="<?=base_url()?>Lembur/detail/'+data+'"><i class="glyphicon glyphicon-eye-open icon-white" title="Detail"></i></a>';   
...
$("#cetakSSPD").click(function(){
    ...
    window.open("<?= base_url()?>Lembur/cetak_sspd_rekap?id_rekap_dana="+id_rekap_dana+"&tgl_sspd="+tgl_sspd);
